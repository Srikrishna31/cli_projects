...
for FILE in $ROOT/*.txt; do
  ...
  tail -c 3     $FILE > ${OUT_FILE}/${BASENAME}.c3.out
  tail -c 8     $FILE > ${OUT_FILE}/${BASENAME}.c8.out
  tail -c 12    $FILE > ${OUT_FILE}/${BASENAME}.c12.out
  tail -c 200   $FILE > ${OUT_FILE}/${BASENAME}.c200.out
